...
function generateSiteConfig(inputConfig) {
  ...
  // Use client images if provided, otherwise fallback to detected images
  const clientLogoUrl = inputConfig.logo_url || logoUrl;
  const clientHeroImage = inputConfig.hero_image || heroImage;
  ...
  const config = {
    ...
    HERO_BACKGROUND_IMAGE: clientHeroImage,
...
function backfill(site) {
  ...
  // Tarifs selon l'offre (PREMIUM uniquement)
  const pack = site.PACK || 'lite';
  if (pack === 'premium') {
    site.ENABLE_PRICING = true;
    if (!Array.isArray(site.PRICING) || site.PRICING.length === 0) {
      site.PRICING = (ph.PRICING || []).slice(0, 5);
    }
  } else {
    site.PRICING = [];
    site.ENABLE_PRICING = false;
  }

  // Galerie selon l'offre (STANDARD et PREMIUM uniquement)
  if (pack !== 'lite') {
    site.ENABLE_GALLERY = true;
    if (!Array.isArray(site.GALLERY) || site.GALLERY.length === 0) {
      site.GALLERY = (ph.GALLERY || []).slice(0, 6);
    }
  } else {
    site.GALLERY = [];
    site.ENABLE_GALLERY = false;
  }

  // Avis selon l'offre (STANDARD et PREMIUM uniquement)
  if (pack !== 'lite') {
    site.ENABLE_REVIEWS = true;
    if (!Array.isArray(site.REVIEWS) || site.REVIEWS.length === 0) {
      site.REVIEWS = (ph.REVIEWS || []).slice(0, 3);
    }
  } else {
    site.REVIEWS = [];
  ...
  // Certifications selon l'offre (PREMIUM uniquement)
  if (pack === 'premium') {
    site.ENABLE_CERTIFICATIONS = true;
    if (!Array.isArray(site.CERTIFICATIONS) || site.CERTIFICATIONS.length === 0) {
      site.CERTIFICATIONS = ph.CERTIFICATIONS || [
        { icon: '✓', name: 'Professionnel Agréé', desc: 'Certification officielle' },
        { icon: '🛡️', name: 'Assurance Pro', desc: 'Couverture complète' },
        { icon: '⭐', name: 'Qualité Garantie', desc: 'Service de qualité' }
      ];
    }
  } else {
    site.CERTIFICATIONS = [];
    site.ENABLE_CERTIFICATIONS = false;
  }
