...
function flattenTallyAnswers(data) {
  ...
  for (const response of data.submission.responses) {
    ...
    const value = response.answer; // Tally uses 'answer' not 'value'
...
async function mapToSiteConfig(tallyData, options = {}) {
  ...
  // Extract basic fields with fallbacks - using real Tally question titles
  const businessName = tallyData['nom-de-votre-entreprise'] || tallyData['business-name'] || 'Mon Entreprise';
  const rawBusinessType = Array.isArray(tallyData['quel-est-votre-metier-principal']) ? 
    tallyData['quel-est-votre-metier-principal'][0] : 
    tallyData['quel-est-votre-metier-principal'];
  const businessType = normalizeNiche(rawBusinessType || tallyData['business-type'] || 'serrurier');
  const city = tallyData['ville-principale-d-intervention'] || tallyData['city'] || 'Toulouse';
  const phone = tallyData['votre-numero-de-telephone'] || tallyData['phone'] || '0612345678';
  const email = tallyData['adresse-email-de-contact'] || tallyData['email'] || 'contact@exemple.fr';
  ...
  // Color
  const accentColor = tallyData['couleur-principale-de-votre-site'] || tallyData['couleur'] || tallyData['accent-color'] || getDefaultColor(businessType);
  ...
  // Handle logo and hero images from Tally
  const hasLogo = Array.isArray(tallyData['avez-vous-un-logo']) ? 
    tallyData['avez-vous-un-logo'][0] === 'Oui' : 
    tallyData['avez-vous-un-logo'] === 'Oui';
  
  const hasHeroImage = Array.isArray(tallyData['voulez-vous-une-image-de-fond-en-haut-du-site']) ? 
    tallyData['voulez-vous-une-image-de-fond-en-haut-du-site'][0] === 'Oui' : 
    tallyData['voulez-vous-une-image-de-fond-en-haut-du-site'] === 'Oui';
  
  // Get the actual file uploads (arrays with file objects)
  const logoField = hasLogo && Array.isArray(tallyData['logo']) ? tallyData['logo'][0] : null;
  const heroField = hasHeroImage && Array.isArray(tallyData['hero']) ? tallyData['hero'][0] : null;
